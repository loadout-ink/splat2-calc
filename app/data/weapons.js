angular.module('splatApp').weapons = function($scope) {
			...
			weapons: [
				{
					...
					verticalInkPerShot: 3.96,
					horizontalInkPerShot: 3.96,
					speedLevel: 'High',
					baseSpeed: 1.28,
					dashSpeed: 1.52,
		...
		},
		{
			...
			weapons: [
				...
				},
				{
					...
					shootingSpeed: 'D',
					minDamage: {
						// NOTE: When calculating min MPU damage both full & partial use the "DamageRate" values!
						full: 16.0,
						partial: 16.0
					},
					maxDamage: {
	...
	$scope.getWeaponBySplatnet = function(splatnetid) {
		for (i = 0; i < $scope.weaponSets.length; i++) {
			for (j = 0; j < $scope.weaponSets[i].weapons.length; j++) {
				if ($scope.weaponSets[i].weapons[j].splatnet == splatnetid) {
					return $scope.weaponSets[i].weapons[j];
				}
			}
		}
		return $scope.weaponSets[0].weapons[0];
	};

	$scope.getWeaponSetBySplatnet = function(splatnetid) {
		for (i = 0; i < $scope.weaponSets.length; i++) {
			for (j = 0; j < $scope.weaponSets[i].weapons.length; j++) {
				if ($scope.weaponSets[i].weapons[j].splatnet == splatnetid) {
					return $scope.weaponSets[i];
				}
			}
		}
		return $scope.weaponSets[0];
	};
};
