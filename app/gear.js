angular.module('splatApp').gear = function($scope) {
	...
	$scope.filterByMain = function(set, abilityName) {
		...
		var secondary = set.filter(function(item) {
			return (
				(item.brand != 'amiibo' &&
					item.brand != 'Cuttlegear' &&
					item.brand != 'Grizzco' &&
					item.main != abilityName) ||
				(item.brand == 'Grizzco' && item.main != abilityName && !$scope.getSkillByName(abilityName).exclusive)
			);
		});
	...
	$scope.isPossibleMain = function(item, abilityName) {
		return (
			...
			(item.brand != 'amiibo' &&
				...
				item.main != abilityName) ||
			(item.brand == 'Grizzco' && item.main != abilityName && !$scope.getSkillByName(abilityName).exclusive) ||
			(item.splatnetShop != null && item.splatnetShop)
		);
	};
	$scope.getHatById = function(id) {
		return (
			$scope.hats.filter(function(item) {
				return item.splatnet == id;
			})[0] || $scope.hats[0]
		);
	};
	$scope.getClothesById = function(id) {
		return (
			$scope.clothes.filter(function(item) {
				return item.splatnet == id;
			})[0] || $scope.clothes[0]
		);
	};
	$scope.getShoesById = function(id) {
		return (
			$scope.shoes.filter(function(item) {
				return item.splatnet == id;
			})[0] || $scope.shoes[0]
		);
	};
