angular
	...
	.controller('splatController', [
		'$scope',
		'$rootScope',
		'$timeout',
		'$locale',
		function splatCtrl($scope, $rootScope, $timeout, $locale, $translate, $uibModal, $log) {
			...
			$scope.resetConditionalAbilities = function() {
				if (
					!$scope.loadout.hasAbility('Opening Gambit') &&
					!$scope.loadout.hasAbility('Last-Ditch Effort') &&
					!$scope.loadout.hasAbility('Comeback') &&
					!$scope.loadout.hasAbility('Drop Roller')
				) {
					$scope.conditionalAbilityCheckbox = false;
				}
			};
			...
			$scope.loadCode = function(code) {
				...
				if (results) {
					$scope.selectedSet = $scope.getWeaponSetBySplatnet(results.weapon);
					newLoadout.weapon = $scope.getWeaponBySplatnet(results.weapon);
			...
			$scope.equip = function(item, slot) {
				if (
					eval('$scope.loadout.' + slot + '.main') == null ||
					eval('$scope.loadout.' + slot + '.equipped.name') == 'Splatfest Tee' ||
					item.name == 'Splatfest Tee'
				) {
			...
			$scope.calcStat = function(abilityScore, weaponType, stat, saveStat) {
				...
				if (stat == 'STAT_SWIM_SPEED') {
					...
					var label = '{{ LABEL_DISTANCE_PER_FRAME | translate }}'.format({
						value: $scope.toFixedTrimmed(value, 4)
					});

					if ($scope.logging) {
						var swim_speed_debug_log = {
				...
				if (stat == 'STAT_RUN_SPEED_FIRING') {
					...
					if ($scope.loadout.weapon.class.toLowerCase() == 'brush') {
						...
						var label = '{{ LABEL_DISTANCE_PER_FRAME | translate }}'.format({
							value: $scope.toFixedTrimmed(result, 4)
						});
					...
					if (
						$scope.loadout.weapon.class.toLowerCase() == 'splatling' ||
						$scope.loadout.weapon.class.toLowerCase() == 'brella'
					) {
						var name = '[+] {{ STAT_RUN_SPEED_FIRING | translate }}';
					} else {
						var name = '{{ STAT_RUN_SPEED_FIRING | translate }}';
					}
					var run_speed_parameters =
						$scope.parameters['Run Speed']['Shooting'][$scope.loadout.weapon.shootingSpeed];
					var abilityScore = $scope.loadout.calcAbilityScore('Run Speed Up');
				...
				if (stat == 'STAT_RUN_SPEED_CHARGING') {
					...
					var percentage = delta;
					var label = '{{ LABEL_DISTANCE_PER_FRAME | translate }}'.format({
						value: $scope.toFixedTrimmed(run_speed, 4)
					});
				...
				if (stat == 'STAT_RUN_SPEED_FLICKING_HORIZONTAL') {
					...
					var delta = ((run_speed / $scope.loadout.weapon.horizontalSwingMoveSpeed - 1) * 100)
						.toFixed(1)
						.toString();
				...
				if (stat == 'STAT_RUN_SPEED_FLICKING_VERTICAL') {
					...
					var delta = ((run_speed / $scope.loadout.weapon.verticalSwingMoveSpeed - 1) * 100)
						.toFixed(1)
						.toString();
				...
				if (stat == 'STAT_RUN_SPEED_FLICKING') {
					...
					var s = $scope.calcS(run_speed_parameters);
					var run_speed =
						$scope.calcRes(run_speed_parameters, p, s) * $scope.loadout.weapon.horizontalSwingMoveSpeed;
					var delta = ((run_speed / $scope.loadout.weapon.horizontalSwingMoveSpeed - 1) * 100)
						.toFixed(1)
						.toString();
				...
				if (stat == 'STAT_SPECIAL_CHARGE') {
					...
					var desc = '{{ DESC_SPECIAL_COST | translate }}'.format({
						value: Math.ceil($scope.loadout.weapon.specialCost / special_charge_speed)
					});
					var label = '{{ LABEL_PERCENT | translate }}'.format({ value: (value * 100).toFixed(1) });

					if ($scope.logging) {
						var special_charge_speed_debug_log = {
							'Special Charge Speed': special_charge_speed,
							AP: abilityScore,
							P: p,
							S: s,
							Delta: percentage
				...
				if (stat == 'STAT_SPECIAL_SAVER_ON_DEATH') {
					...
					if ($scope.logging) {
						var special_saver_debug_log = {
							'Special Saver (On Death)': special_saved,
							AP: ap,
							Delta: modifier
						};
						console.log(special_saver_debug_log);
				...
				if (stat == 'STAT_SUB_POWER_RANGE') {
					...
					switch (equippedSub.name) {
						case 'Point Sensor':
							...
							var value = $scope.toFixedTrimmed(result / max_param * 100, 2);
							var percentage = ((result / min_param - 1) * 100).toFixed(1);
							var label = '{{ LABEL_NO_UNIT | translate }}'.format({
								value: $scope.toFixedTrimmed(result, 2)
							});

							if ($scope.logging) {
								var sub_power_up_debug_log = {
									'Sub Power Up (Point Sensor Range)': result,
									'AP:': abilityScore,
									P: p,
									S: s,
									'Delta:': this.percentage
								};
				...
				if (stat == 'STAT_SUB_POWER_TRACKING_TIME') {
					...
					if (equippedSub.name == 'Point Sensor') {
						...
						if ($scope.logging) {
							var main_power_up_debug_log = {
								'Sub Power Up (Point Sensor Tracking Time)': result,
								'AP:': abilityScore,
								P: p,
								S: s,
								'Delta:': percentage
					...
					if (equippedSub.name == 'Ink Mine') {
						...
						if ($scope.logging) {
							var main_power_up_debug_log = {
								'Sub Power Up (Ink Mine Tracking Time)': result,
								'AP:': abilityScore,
								P: p,
								S: s,
								'Delta:': percentage
				...
				if (stat == 'STAT_SUB_POWER_MINE') {
					...
					var value = $scope.toFixedTrimmed(result / max_param * 100, 2);
					var percentage = ((result / min_param - 1) * 100).toFixed(1);
					var label = '{{ LABEL_PERCENT | translate }}'.format({
						value: $scope.toFixedTrimmed(result * 100, 2)
					});

					if ($scope.logging) {
						var sub_power_up_debug_log = {
							'Sub Power Up (Ink Mine Radius)': result,
							'AP:': abilityScore,
							P: p,
							S: s,
							'Delta:': percentage
				...
				if (stat == 'STAT_SAVER_MAIN') {
					...
					if ($scope.loadout.weapon.class == 'Roller' || $scope.loadout.weapon.class == 'Brush') {
						...
						var desc = '{{ DESC_MAIN_COST | translate }}'.format({
							totalShots: Math.floor(100 / costPerShot),
							reduction: (100 - reduction * 100).toFixed(1)
						});
						var label = '{{ INK_BREAKDOWN_INK_PER_SECOND | translate }}'.format({
							value: $scope.toFixedTrimmed(costPerShot, 3)
						});
					} else {
						...
						var label = '{{ LABEL_MAIN_COST | translate }}'.format({
							value: $scope.toFixedTrimmed(costPerShot, 3),
							unit: $scope.loadout.weapon.shotUnit
						});
					}

					if ($scope.loadout.weapon.name.indexOf('Splattershot Jr.') !== -1) {
						var desc = '{{ DESC_MAIN_COST | translate }}'.format({
							totalShots: Math.floor(110 / costPerShot),
							reduction: (100 - reduction * 100).toFixed(1)
						});
					}
					...
					if ($scope.logging) {
						var ink_saver_debug_log = {
							'Ink Saver (Main)': costPerShot,
							AP: abilityScore,
							P: p,
							S: s,
							Delta: reduction
				...
				if (stat == 'STAT_SAVER_MAIN_FLICK') {
					...
					var label = '{{ LABEL_MAIN_COST | translate }}'.format({
						value: $scope.toFixedTrimmed(costPerShot, 3),
						unit: '{{SHOT_UNIT_FLICK | translate}}'
					});
					var value = costPerShot;
					var percentage = (100 - reduction * 100).toFixed(1);
				...
				if (stat == 'STAT_SAVER_MAIN_HORIZONTAL_FLICK') {
					...
					var label = '{{ LABEL_MAIN_COST | translate }}'.format({
						value: $scope.toFixedTrimmed(costPerShot, 3),
						unit: '{{SHOT_UNIT_FLICK_HORIZ | translate}}'
					});
					var value = costPerShot;
					var percentage = (100 - reduction * 100).toFixed(1);
				...
				if (stat == 'STAT_SAVER_MAIN_VERTICAL_FLICK') {
					...
					var label = '{{ LABEL_MAIN_COST | translate }}'.format({
						value: $scope.toFixedTrimmed(costPerShot, 3),
						unit: '{{SHOT_UNIT_FLICK_VERT | translate}}'
					});
					var value = costPerShot;
					var percentage = (100 - reduction * 100).toFixed(1);
				...
				if (stat == 'STAT_SAVER_SUB') {
					...
					var desc = '{{ DESC_SUB_COST | translate }}'.format({
						reduction: (100 - reduction * 100).toFixed(1)
					});
					var label = '{{ LABEL_SUB_COST | translate }}'.format({
						value: $scope.toFixedTrimmed(costPerSub, 3)
					});
					var value = costPerSub;
					var percentage = (100 - reduction * 100).toFixed(1);

					if ($scope.logging) {
						var ink_saver_sub_debug_log = {
							'Ink Saver (Sub)': costPerSub,
							AP: abilityScore,
							P: p,
							S: s,
							Delta: reduction
				...
				if (stat == 'STAT_RECOVERY_SQUID') {
					...
					if ($scope.logging) {
						var refill_speed_squid_debug_log = {
							'Ink Recovery Speed (Squid)': refill_rate,
							time: refill_time,
							AP: abilityScore,
							P: p,
							S: s,
				...
				if (stat == 'STAT_RECOVERY_KID') {
					...
					if ($scope.logging) {
						var refill_speed_squid_debug_log = {
							'Ink Recovery Speed (Kid)': refill_rate,
							time: refill_time,
							AP: abilityScore,
							P: p,
							S: s,
				...
				if (weaponType == '.52 Gal') {
					if (stat == 'STAT_MAIN_POWER_UP_JUMP_SHOT_RANDOMIZATION') {
						...
						var label = '{{ LABEL_PERCENT | translate }}'.format({
							value: $scope.toFixedTrimmed(result / min_param * 100, 3)
						});

						if ($scope.logging) {
							var main_power_up_debug_log = {
								'Main Power Up': result,
								'AP:': abilityScore,
								P: p,
								S: s,
								'Delta:': percentage
				...
				if (weaponType == '.96 Gal') {
					if (stat == 'STAT_MAIN_POWER_UP_MIN_DAMAGE') {
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage != null &&
						...
						var percentage = Math.abs(((result / min_param - 1) * 100).toFixed(1));
						var label = '{{ LABEL_NO_UNIT | translate }}'.format({
							value: $scope.toFixedTrimmed(min_damage, 2)
						});
					...
					if (stat == 'STAT_MAIN_POWER_UP_MAX_DAMAGE') {
						...
						var percentage = Math.abs(((result / min_param - 1) * 100).toFixed(1));
						var label = '{{ LABEL_NO_UNIT | translate }}'.format({
							value: $scope.toFixedTrimmed(max_damage, 2)
						});
				...
				if (weaponType == 'Ballpoint Splatling') {
					if (stat == 'STAT_MAIN_POWER_UP_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Ballpoint Splatling']['DamageRate']['normal_params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.normal != null &&
					...
					if (stat == 'STAT_MAIN_POWER_UP_REPEAT_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Ballpoint Splatling']['DamageRate']['repeat_params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.repeat != null &&
				...
				if (weaponType == 'Bamboozler 14') {
					if (stat == 'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Bamboozler 14']['MinMaxChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.partial != null &&
					...
					if (stat == 'STAT_MAIN_POWER_UP_FULL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Bamboozler 14']['FullChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.full != null &&
				...
				if (weaponType == 'Bloblobber') {
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Bloblobber']['SplashPaintRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
						...
						var percentage = Math.abs(((result / min_param - 1) * 100).toFixed(1));
						var label = '{{ LABEL_PERCENT | translate }}'.format({
							value: $scope.toFixedTrimmed(result * 100, 3)
						});
				...
				if (weaponType == 'Carbon Roller') {
					if (stat == 'STAT_MAIN_POWER_UP_ROLLING_MIN_DAMAGE') {
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.rolling != null &&
					...
					if (stat == 'STAT_MAIN_POWER_UP_STANDING_MIN_DAMAGE') {
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.standing != null &&
					...
					if (stat == 'STAT_MAIN_POWER_UP_JUMPING_MIN_DAMAGE') {
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.jumping != null &&
				...
				if (weaponType == 'Dualie Squelchers') {
					if (stat == 'STAT_MAIN_POWER_UP_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Dualie Squelchers']['DamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'E-liter 4K Scope') {
					if (stat == 'STAT_MAIN_POWER_UP_FULL_CHARGE_DISTANCE') {
						var parameters =
							$scope.parameters['Main Power Up']['E-liter 4K Scope']['FullChargeDistance']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_DISTANCE') {
						var parameters =
							$scope.parameters['Main Power Up']['E-liter 4K Scope']['MaxDistance']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['E-liter 4K Scope']['SplashPaintRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'E-liter 4K') {
					if (stat == 'STAT_MAIN_POWER_UP_FULL_CHARGE_DISTANCE') {
						var parameters =
							$scope.parameters['Main Power Up']['E-liter 4K']['FullChargeDistance']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['E-liter 4K']['SplashPaintRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Explosher') {
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Explosher']['SplashPaintRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Glooga Dualies') {
					...
					if (stat == 'STAT_MAIN_POWER_UP_BURST_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Glooga Dualies']['SideStepOneMuzzleDamageRate'][
								'params'
							];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						...
						if (
							$scope.loadout.weapon.mpuMaxDamage.burst != null &&
				...
				if (weaponType == 'Goo Tuber') {
					if (stat == 'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Goo Tuber']['MinMaxChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_FULL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Goo Tuber']['FullChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Heavy Splatling') {
					if (stat == 'STAT_MAIN_POWER_UP_FIRST_RING_BURST_DURATION') {
						var parameters =
							$scope.parameters['Main Power Up']['Heavy Splatling']['MaxChargeShootingFrameTimes'][
								'params'
							];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						...
						var value = $scope.toFixedTrimmed(result / max_param * 100, 2);
						var percentage = ((result / min_param - 1) * 100).toFixed(1);
						var label = '{{ LABEL_TIME | translate }}'.format({
							value: $scope.toFixedTrimmed(result * $scope.loadout.weapon.firstRingBurstDuration, 2)
						});
					...
					if (stat == 'STAT_MAIN_POWER_UP_SECOND_RING_BURST_DURATION') {
						...
						var percentage = ((result / min_param - 1) * 100).toFixed(1);
						var label = '{{ LABEL_TIME | translate }}'.format({
							value: $scope.toFixedTrimmed(result * $scope.loadout.weapon.secondRingBurstDuration, 2)
						});
				...
				if (weaponType == 'Hydra Splatling') {
					...
					if (stat == 'STAT_MAIN_POWER_UP_FULL_CHARGE_MAX_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Hydra Splatling']['DamageMaxMaxChargeRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Inkbrush') {
					...
					if (stat == 'STAT_MAIN_POWER_UP_INK_TRAIL_SIZE') {
						var parameters =
							$scope.parameters['Main Power Up']['Inkbrush']['CorePaintWidthHalfRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Luna Blaster') {
					if (stat == 'STAT_MAIN_POWER_UP_HIGH_DAMAGE_RADIUS') {
						var parameters =
							$scope.parameters['Main Power Up']['Luna Blaster']['CollisionRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Luna Blaster']['SphereSplashDropPaintRadiusRate'][
								'params'
							];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
				...
				if (weaponType == 'Mini Splatling') {
					if (stat == 'STAT_MAIN_POWER_UP_FIRST_RING_BURST_DURATION') {
						var parameters =
							$scope.parameters['Main Power Up']['Mini Splatling']['MaxChargeShootingFrameTimes'][
								'params'
							];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
				...
				if (weaponType == 'Nautilus') {
					if (stat == 'STAT_MAIN_POWER_UP_FIRST_RING_BURST_DURATION') {
						var parameters =
							$scope.parameters['Main Power Up']['Nautilus']['MaxChargeShootingFrameTimes']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s) / 60;
						var max_param = parameters[0] / 60;
				...
				if (weaponType == 'Octobrush') {
					...
					if (stat == 'STAT_MAIN_POWER_UP_INK_TRAIL_SIZE') {
						var parameters =
							$scope.parameters['Main Power Up']['Octobrush']['CorePaintWidthHalfRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Rapid Blaster Pro') {
					if (stat == 'STAT_MAIN_POWER_UP_JUMP_SHOT_RANDOMIZATION') {
						var parameters =
							$scope.parameters['Main Power Up']['Rapid Blaster Pro']['DegJumpRandom']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_DAMAGE_RADIUS') {
						var parameters =
							$scope.parameters['Main Power Up']['Rapid Blaster Pro']['CollisionRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_value =
							$scope.parameters['Main Power Up']['Rapid Blaster Pro']['CollisionRadiusRate'][
								'CollisionRadiusMiddle_MWPUG_Max'
							];
						var min_value =
							$scope.parameters['Main Power Up']['Rapid Blaster Pro']['CollisionRadiusRate'][
								'CollisionRadiusMiddle'
							];
						...
						var label = '{{ LABEL_PERCENT | translate }}'.format({
							value: $scope.toFixedTrimmed(result / min_value * 100, 3)
						});
				...
				if (weaponType == 'Rapid Blaster') {
					...
					if (stat == 'STAT_MAIN_POWER_UP_DAMAGE_RADIUS') {
						var parameters =
							$scope.parameters['Main Power Up']['Rapid Blaster']['CollisionRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_value =
							$scope.parameters['Main Power Up']['Rapid Blaster']['CollisionRadiusRate'][
								'CollisionRadiusMiddle_MWPUG_Max'
							];
						var min_value =
							$scope.parameters['Main Power Up']['Rapid Blaster']['CollisionRadiusRate'][
				...
				if (weaponType == 'Splat Charger') {
					if (stat == 'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Splat Charger']['MinMaxChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_FULL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Splat Charger']['FullChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Splatterscope') {
					if (stat == 'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Splatterscope']['MinMaxChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
					...
					if (stat == 'STAT_MAIN_POWER_UP_FULL_CHARGE_MIN_DAMAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Splatterscope']['FullChargeDamageRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Splattershot Jr.') {
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Splattershot Jr.']['SplashPaintRadius']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Squiffer') {
					...
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Squiffer']['SplashPaintRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Tri-Slosher') {
					if (stat == 'STAT_MAIN_POWER_UP_INK_COVERAGE') {
						var parameters =
							$scope.parameters['Main Power Up']['Tri-Slosher']['SplashPaintRadiusRate']['params'];
						var p = $scope.calcP(abilityScore);
						var s = $scope.calcS(parameters);
						var result = $scope.calcRes(parameters, p, s);
						var max_param = parameters[0];
				...
				if (weaponType == 'Undercover Brella') {
					var parameters =
						$scope.parameters['Main Power Up']['Undercover Brella']['CanopyNakedFrame']['params'];
					var p = $scope.calcP(abilityScore);
					var s = $scope.calcS(parameters);
					var result = $scope.calcRes(parameters, p, s) / 60;
					var max_param = parameters[0] / 60;
			...
			$scope.statBarClicked = function(name) {
				if ($scope.loadout.weapon.type == 'Ballpoint Splatling') {
					if (name == '[+] {{ STAT_MAIN_POWER_UP_MIN_DAMAGE | translate }}') {
						var abilityScore = $scope.loadout.calcAbilityScore('Main Power Up');
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_REPEAT_MIN_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_MAX_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_REPEAT_MAX_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 2',
				...
				if ($scope.loadout.weapon.type == 'Bamboozler 14') {
					if (name == '[+] {{ STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MIN_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_FULL_CHARGE_MIN_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MAX_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_FULL_CHARGE_MAX_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 2',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_FULL_CHARGE_MIN_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MIN_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_FULL_CHARGE_MAX_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_MAX_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 2',
				...
				if ($scope.loadout.weapon.type == 'Carbon Roller') {
					if (name == '[+] {{ STAT_MAIN_POWER_UP_ROLLING_MIN_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_STANDING_MIN_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_ROLLING_MAX_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_STANDING_MAX_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 2',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_STANDING_MIN_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_JUMPING_MIN_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_STANDING_MAX_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_JUMPING_MAX_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 2',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_JUMPING_MIN_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_ROLLING_MIN_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_JUMPING_MAX_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_ROLLING_MAX_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 2',
				...
				if ($scope.loadout.weapon.type == 'E-liter 4K Scope') {
					if (name == '[+] {{ STAT_MAIN_POWER_UP_FULL_CHARGE_DISTANCE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_PARTIAL_CHARGE_DISTANCE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_PARTIAL_CHARGE_DISTANCE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_FULL_CHARGE_DISTANCE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
				...
				if ($scope.loadout.weapon.type == 'Glooga Dualies') {
					if (name == '[+] {{ STAT_MAIN_POWER_UP_MIN_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_BURST_MIN_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 1',
					...
					if (name == '[+] {{ STAT_MAIN_POWER_UP_MAX_DAMAGE | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_MAIN_POWER_UP_BURST_MAX_DAMAGE',
							true
						);
						$scope.displayStat(
							'Main Power Up 2',
         ...
         **************************/
					if (name == '[+] {{ STAT_SAVER_MAIN_ROLLING | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_SAVER_MAIN_HORIZONTAL_FLICK',
							true
						);
						$scope.displayStat(
							'Ink Consumption (Main)',
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_SAVER_MAIN_VERTICAL_FLICK',
							true
						);
						$scope.displayStat(
							'Ink Consumption (Main)',
         ...
         ****************************/
					if (name == '[+] {{ STAT_RUN_SPEED_ROLLING | translate }}') {
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_RUN_SPEED_FLICKING_HORIZONTAL',
							true
						);
						$scope.displayStat(
							'Run Speed (Firing)',
						...
						var statValues = $scope.calcStat(
							abilityScore,
							$scope.loadout.weapon.type,
							'STAT_RUN_SPEED_FLICKING_VERTICAL',
							true
						);
						$scope.displayStat(
							'Run Speed (Firing)',
         ...
         ****************************/
				...
				switch (name) {
					case '[+] {{ STAT_TRACKING_TIME_POINT_SENSOR | translate }}':
						...
						$scope.stats['Tracking Time'].label = '{{ LABEL_TIME | translate }}'.format({
							value: $scope.toFixedTrimmed(duration, 2)
						});
			...
			$scope.parameters = {
				...
				'Main Power Up': {
					...
					'.96 Gal': {
						DamageRate: {
							desc: 'Damage Up',
							params: [ 1.25, 1.125, 1.0 ]
						}
					},
					Aerospray: {
						SplashPaintRadius: {
							desc: 'Increased Ink Coverage',
							params: [ 14.0, 13.3, 12.0 ]
						}
					},
					'Ballpoint Splatling': {
						DamageRate: {
							desc: 'Damage Up',
							normal_params: [ 1.1, 1.05, 1.0 ],
							repeat_params: [ 1.1, 1.05, 1.0 ]
						}
					},
					'Bamboozler 14': {
					...
					'Carbon Roller': {
						DamageRate: {
							desc: 'Damage Up',
							rolling: [ 1.15, 1.075, 1.0 ],
							standing: [ 1.15, 1.075, 1.0 ],
							jumping: [ 1.15, 1.075, 1.0 ]
						}
					},
					'Clash Blaster': {
						DegJumpRandom: {
					...
					'E-liter 4K Scope': {
						FullChargeDistance: {
							...
							params: [ 325.5, 318.0, 310.5 ]
						},
						MaxDistance: {
							desc: 'Partial Charge Distance',
							params: [ 305.5, 298.0, 290.5 ]
						},
						SplashPaintRadiusRate: {
							desc: 'Increased Ink Coverage',
							params: [ 1.17, 1.12, 1.0 ]
						}
					},
					'E-liter 4K': {
						FullChargeDistance: {
					...
					'Flingza Roller': {
						DamageRate: {
							desc: 'Damage Up',
							rolling: [ 1.15, 1.1, 1.0 ],
							standing: [ 1.15, 1.1, 1.0 ],
							jumping: [ 1.15, 1.1, 1.0 ]
						}
					},
					'Glooga Dualies': {
						DamageRate: {
					...
					Inkbrush: {
						DashSpeed: {
							desc: 'Dash Speed',
							params: [ 2.016, 1.986, 1.92 ]
						},
						CorePaintWidthHalfRate: {
							desc: 'Ink Trail Width',
							params: [ 1.48, 1.32, 1.0 ]
						}
					},
					'Jet Squelcher': {
						DegRandom: {
					...
					'Luna Blaster': {
						...
						SphereSplashDropPaintRadiusRate: {
							desc: 'Increased Ink Coverage',
							params: [ 1.09375, 1.05625, 1.0 ]
						}
					},
					'Mini Splatling': {
						MaxChargeShootingFrameTimes: {
							desc: 'Burst Duration',
							params: [ 1.35, 1.25, 1.0 ]
						}
					},
					'N-ZAP': {
						SplashPaintRadius: {
							desc: 'Increased Ink Coverage',
							params: [ 13.2, 12.6, 11.6 ]
						},
						DegRandom: {
							desc: 'Ground Shot Randomization',
							params: [ 4.8, 5.2, 6.0 ]
					...
					'Splattershot Jr.': {
						SplashPaintRadius: {
							desc: 'Increased Ink Coverage',
							params: [ 15.5, 13.7, 12.9 ]
						}
					},
					'Splattershot Pro': {
						DamageRate: {
							desc: 'Damage Up',
							params: [ 1.216, 1.108, 1.0 ]
						}
					},
					Splattershot: {
						DegRandom: {
					...
					Squeezer: {
						DamageRate: {
							desc: 'Damage Up',
							normal_params: [ 1.3, 1.15, 1.0 ],
							repeat_params: [ 1.2, 1.1, 1.0 ]
						}
					},
					Squiffer: {
						FullChargeDistance: {
							desc: 'Full Charge Distance',
							params: [ 182.65, 175.15, 167.65 ]
						},
						MaxDistance: {
							desc: 'Partial Charge Distance',
							params: [ 182.65, 175.15, 167.65 ]
